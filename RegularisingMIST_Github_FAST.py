# ---------------------------------------------------------------------------------
# Written by Samantha Alloo -- 28/06/2025
# Updates version that uses the scipy.optimize.minimize finction to determine optimal regularisation
...
savedir = os.path.join(data, "RetrievedSignals_Revised") # Combine the base directory and new folder name to create the full path
...
def find_optimal_epsilon(epsilon_initial, phase_TIE, kr2, ftimage):
    # Find the optimal epsilon value for phase reconstruction using MSE.

    # Parameters
    # ----------
    # epsilon_initial : float
    #     Initial guess for the epsilon value.
    # phase_TIE : np.ndarray
    #     Ground truth phase reconstruction from Pavlov et al. TIE.
    # kr2 : np.ndarray
    #     Sum of squared Fourier space spatial frequencies.
    # ftimage : np.ndarray
    #     Image to which the (1/kr^2) filter is applied.

    # Returns
    # -------
    # best_fine_phase : np.ndarray
    #     Optimal phase reconstruction after fine-tuning.
    # best_fine_epsilon : float
    #     Optimal epsilon value after fine-tuning.
    def mse_loss(epsilon):
        regdiv = 1 / (kr2 + epsilon)  # Regularization division
        phase_FP_it = np.real(
            -1 * np.fft.ifft2(regdiv * ftimage)
        )  # Inverse FFT and filter applied for phase reconstruction

        # Compute the mean square error of the two phase images
        phase_FP_it_crop = phase_FP_it[:phase_TIE.shape[0],:phase_TIE.shape[1]] # need to crop so images have same shape
        squared_diff = (phase_TIE - phase_FP_it_crop) ** 2
        MSE = np.mean(squared_diff, dtype=np.float64)
        return MSE

    # Use the minimize function to find the optimal epsilon
    ...
    # Extract the optimal epsilon and phase from the result
...
optimal_phase_FP_MSE, optimal_epsilon_MSE = find_optimal_epsilon(epsilon_inital, phase_TIE, kr2, ftimage) # This runs the iterative function defined above
