...
def EpsilonOpt_IterativeAlgorithm_MSE(epsilon_initial, phase_TIE, kr2, ftimage):
    ...
    # Get the order of magnitude of the best epsilon
    epsilon_order = np.floor(np.log10(best_epsilon))
    initial_epsilon_fine = 10 ** epsilon_order
    ...
    epsilon_fine = initial_epsilon_fine
    fine_iteration = 0
    restarted = False  # Flag to track whether we've already restarted

    while True:
        # Compute the MIST phase reconstruction
        ...
        if MSE_fine > previous_MSE_fine:
            if fine_iteration == 1 and not restarted:
                # Restart with smaller epsilon and step size if the initial global MSE minimum is overshot
                print(f"MSE increased on first fine-tune step. Restarting from smaller epsilon.")
                epsilon_order -= 1
                epsilon_fine = 10 ** epsilon_order
                # Uncomment the line below to reduce the step size if MSE sensitivity is too low —
                # for example, if MSE varies significantly near the minimum, suggesting the global minimum may be missed.
                #epsilon_step = 10 ** (epsilon_order - 3)
                previous_MSE_fine = float('inf')
                fine_iteration = 0
                restarted = True
                ...
                print(f"MSE increased for epsilon = {epsilon_fine}. Optimal regularisation found and fine-tuning is stopping")
                break
